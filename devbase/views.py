...
def like_view(request,pk):
    ...
    if post.likes.filter(id = request.user.id).exists():
        ...
        liked = True
    return HttpResponseRedirect(reverse('devbase:question_detail',args=[str(pk)]))
...
class QuestionListView(ListView):
    ...
    #     if search_input:
    #         context['questions'] = context['questions'].filter(title_icontains = search_input)
...
class CoomentDetailView(CreateView):
    model = Comment
    form_class = CommentForm
    template_name = "devbase:question_detail.html"
    ...
    success_url = reverse_lazy('devbase:question_detail')


class AddCommentView(CreateView):
    model = Comment
    form_class = CommentForm
    template_name = 'devbase/question_answer.html'

    def form_valid(self, form):
        form.instance.question_id = self.kwargs['pk']
        return super().form_valid(form)
        
    success_url = reverse_lazy('devbase:question_list')